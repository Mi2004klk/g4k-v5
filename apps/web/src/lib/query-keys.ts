...
export const queryKeys = {
  ...
  projects: (search?: string, sort?: string, page?: string) => search !== undefined ? ["projects", search, sort ?? "", page ?? "1"] as const : ["projects"] as const,
  tasks: (projectId?: string) => projectId !== undefined ? ["tasks", projectId] as const : ["tasks"] as const,
  tasksSubmitted: ["tasks-submitted"] as const,
  taskDetail: (id: number | string) => ["task-detail", id] as const,
  projectTasks: (id: string | number) => ["project-tasks", String(id)] as const,
  projectsCount: ["projects", "count"] as const,
  attendanceTeamToday: ["attendance", "team-today"] as const,
  adminLeaveHistory: ["admin_leave_history"] as const,
  taskById: (id: string | number) => ["tasks", String(id)] as const,
  ...
  myLeaveHistoryKey: ["my-leave-history"] as const,
  ...
  systemPublicConfig: ["system_public_config"] as const,
  adminJobs: ["admin_jobs"] as const,
  userPreferences: ["user-preferences"] as const,
  demoDataStatus: ["demo-data-status"] as const,
  tasksAll: ["tasks", "all"] as const,
  projectPhases: (projectId: string | number) => ["project-phases", String(projectId)] as const,
