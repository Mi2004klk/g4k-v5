...
function Calendar({
  ...
  const onTouchEnd = (e: React.TouchEvent) => {
    ...
    if (isLeftSwipe || isRightSwipe) {
      ...
      if (isLeftSwipe) {
        const nextBtn = container.querySelector('button[aria-label="Go to next month"]') as HTMLButtonElement | null;
        if (nextBtn) nextBtn.click();
      } else {
        const prevBtn = container.querySelector('button[aria-label="Go to previous month"]') as HTMLButtonElement | null;
      ...
      <DayPicker
        ...
        components={{
          Chevron: ({ orientation, className, ...props }: any) => {
            ...
            return (
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={cn("h-4 w-4", isRight ? "rotate-180" : isUp ? "rotate-90" : isDown ? "-rotate-90" : "", className)} {...props}>
