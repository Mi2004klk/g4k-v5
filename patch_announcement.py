...
with open('apps/web/src/components/widgets/announcement-board.tsx', 'r') as f:
    content = f.read()
...
content = content.replace('value={createData.title}', 'value={activeCreateData.title}')
content = content.replace('onChange={(e) => setCreateData(prev => ({ ...prev, title: e.target.value }))}', 'onChange={(e) => { setCreateData(prev => ({ ...prev, title: e.target.value })); handleFieldChange({ title: e.target.value }); }}')

content = content.replace('value={createData.body}', 'value={activeCreateData.body}')
content = content.replace('onChange={(e) => setCreateData(prev => ({ ...prev, body: e.target.value }))}', 'onChange={(e) => { setCreateData(prev => ({ ...prev, body: e.target.value })); handleFieldChange({ body: e.target.value }); }}')

content = content.replace('value={createData.scope}', 'value={activeCreateData.scope}')
content = content.replace('onValueChange={(v) => setCreateData(prev => ({ ...prev, scope: v }))}', 'onValueChange={(v) => { setCreateData(prev => ({ ...prev, scope: v })); handleFieldChange({ scope: v }); }}')

content = content.replace('checked={createData.pinned}', 'checked={activeCreateData.pinned}')
content = content.replace('onCheckedChange={(c) => setCreateData(prev => ({ ...prev, pinned: !!c }))}', 'onCheckedChange={(c) => { setCreateData(prev => ({ ...prev, pinned: !!c })); handleFieldChange({ pinned: !!c }); }}')
...
# Need to change mutation body to use activeCreateData if doing create, but createData if doing update!
# Let's find the save mutation body
...
with open('apps/web/src/components/widgets/announcement-board.tsx', 'w') as f:
    f.write(content)
